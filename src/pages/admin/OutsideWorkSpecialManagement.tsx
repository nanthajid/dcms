...
const OutsideWorkSpecialManagement: React.FC = () => {
  ...
  const fetchOutsideWork = async () => {
    ...
    try {
      ...
      toast.error('ไม่สามารถโหลดข้อมูลการทำงานนอก (กรณีพิเศษ) ได้');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchHolidays = async () => {
    try {
      const response = await axios.get('/dcms/api/admin/holidays_special_management.php?action=list');
      if (response.data.success) {
  ...
  const fetchReportTitles = async () => {
    try {
      const response = await axios.get('/dcms/api/admin/report_configuration.php?action=list');
      if (response.data.success) {
        setReportTitles(response.data.data?.titles || []);
        if (response.data.data?.selectedId) {
          setSelectedReportTitleId(response.data.data.selectedId);
        }
  ...
  const handleAddReportTitle = async () => {
    ...
    try {
      const response = await axios.post('/dcms/api/admin/report_configuration.php?action=add', {
      ...
      toast.error('เพิ่มหัวข้อรายงานไม่สำเร็จ');
    }
  };
  ...
  const handleUpdateReportTitle = async () => {
    ...
    try {
      const response = await axios.post('/dcms/api/admin/report_configuration.php?action=update', {
        id: editTitleForm.id,
        title: editTitleForm.title
      });
      if (response.data.success) {
      ...
      toast.error('แก้ไขหัวข้อรายงานไม่สำเร็จ');
    }
  };

  const handleDeleteReportTitle = async (id: string) => {
    try {
      const response = await axios.post('/dcms/api/admin/report_configuration.php?action=delete', { id });
      if (response.data.success) {
      ...
      toast.error('ลบหัวข้อรายงานไม่สำเร็จ');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post('/dcms/api/admin/outside_work_special_management.php?action=save', {
        ...formData,
  ...
  const handleDelete = async (id: string) => {
    setConfirmModal({
      ...
      onConfirm: async () => {
        try {
          const response = await axios.post('/dcms/api/admin/outside_work_special_management.php?action=delete', { id });
          if (response.data.success) {
            toast.success(response.data.message);
            fetchOutsideWork();
          }
  ...
  const handleUpdateRecord = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post('/dcms/api/admin/outside_work_special_management.php?action=update', {
        id: editRecord.id,
        hours,
  ...
  const handleSaveHoliday = async (dataToSave?: any) => {
    try {
      ...
      const response = await axios.post('/dcms/api/admin/holidays_special_management.php?action=save', payload);
      if (response.data.success) {
        toast.success(response.data.message);
  ...
  const handleDeleteHoliday = async (id: string) => {
    try {
      const response = await axios.post('/dcms/api/admin/holidays_special_management.php?action=delete', { id });
      if (response.data.success) {
        toast.success(response.data.message);
        fetchHolidays();
      }
  ...
  const handleAutoGenerate = async () => {
    ...
    setConfirmModal({
      open: true,
      title: 'ยืนยันการลงเวลาอัตโนมัติ (กรณีพิเศษ)',
      message: (
        <div className="space-y-4 text-left">
          ...
          <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100">
            ...
            <p className="text-[10px] text-emerald-700 mb-3">
              มีผลเฉพาะการลงเวลารอบนี้ (เดือน/ปี ช่วงวันที่ และกลุ่มที่เลือกด้านบน) — ไม่กระทบค่าเริ่มต้นในเมนู "จัดการค่าธรรมเนียม"
            </p>
      ...
      onConfirm: async () => {
        ...
        try {
          const response = await axios.post('/dcms/api/admin/outside_work_special_management.php?action=auto_generate', {
            month,
            year,
            start_day: startDay,
            end_day: endDay,
  ...
  const handleResetMonth = () => {
    ...
    setConfirmModal({
      ...
      message: (
        <div className="space-y-4 text-left">
          <p className="text-gray-600 font-medium">
            กรุณาเลือกเดือนและปีที่ต้องการลบข้อมูลการทำงานนอกเวลา (กรณีพิเศษ) <br/>
      ...
      onConfirm: async () => {
        ...
        try {
          const response = await axios.post('/dcms/api/admin/outside_work_special_management.php?action=clear', {
            month,
            year
          });
          if (response.data.success) {
  ...
  const handleBulkDelete = async () => {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        try {
          const response = await axios.post('/dcms/api/admin/outside_work_special_management.php?action=bulk_delete', { ids: selectedIds });
          if (response.data.success) {
            toast.success(response.data.message);
            setRowSelection({});
            fetchOutsideWork();
  ...
  const generateSummaryPDF = async () => {
    ...
    try {
      ...
      const selectedTitle = reportTitles?.find(t => t.id === selectedReportTitleId)?.title || 'รายงานสรุปการเบิกเงินค่าตอบแทนนอกเวลาราชการ (กรณีพิเศษ)';
      ...
      link.download = `รายงานสรุป_ค่าตอบแทนนอกเวลา(กรณีพิเศษ)_${thaiMonths[selectedMonth - 1]}${rangeSuffix}_${selectedYear + 543}.pdf`;
      link.click();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">จัดการการทำงานนอกเวลา (กรณีพิเศษ)</h2>
          <p className="text-gray-500">บันทึกและตรวจสอบตารางการปฏิบัติงานนอกเวลาราชการ กรณีพิเศษ</p>
        </div>
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          ...
          <button
            onClick={() => navigate('/admin/outside-work-special-leave')}
          ...
          <button
            ...
            className="flex-1 md:flex-none bg-blue-50 text-blue-600 px-6 py-2.5 rounded-xl hover:bg-blue-600 hover:text-white transition-all flex items-center justify-center gap-2 font-bold border border-blue-200"
          >
            <FileText size={20} />
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div>
              <h3 className="font-bold text-gray-800 text-lg">รายงานค่าตอบแทน (กรณีพิเศษ)</h3>
              <p className="text-sm text-gray-500">เลือกเดือนที่ต้องการส่งออก PDF</p>
            </div>
          </div>
                ...
                {table.getRowModel().rows.length === 0 ? (
                  <tr>
                    <td colSpan={columns.length} className="px-6 py-10 text-center text-gray-400">
                      ไม่พบข้อมูลการทำงานนอกเวลา (กรณีพิเศษ) ในเดือนนี้
                    </td>
                  </tr>
                ) : (
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              <div className="grid grid-cols-1 gap-6">
                ...
                <div className="bg-purple-50 p-4 rounded-xl border border-purple-100 text-sm text-purple-800">
                  <p className="font-bold mb-1">หมายเหตุ (กรณีพิเศษ):</p>
                  <p>บันทึกวันที่มาปฏิบัติงาน โดยจำนวนชั่วโมง (ชม.) = 0</p>
                </div>
              </div>
              ...
              <div className="flex gap-4 pt-6 border-t border-gray-100">
                ...
                <button type="submit" className="flex-[2] py-3 bg-purple-700 text-white rounded-xl font-bold hover:bg-purple-800 transition-all shadow-lg shadow-purple-700/20">บันทึกข้อมูล</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {isLeaveModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200">
            <div className="bg-orange-600 p-6 text-white flex justify-between items-center flex-shrink-0">
              <h3 className="text-xl font-bold flex items-center gap-2">
                <CalendarIcon size={20} />
            ...
            <form onSubmit={handleSubmitLeave} className="overflow-y-auto flex-grow p-8 space-y-5">
              <div>
                ...
                {(() => {
                  const uniqueStIds = [...new Set(rawData.map(item => item.StID))];
                  ...
                  <div className="flex flex-wrap gap-2 mt-3 p-3 bg-orange-50 rounded-xl border border-orange-100 max-h-32 overflow-y-auto">
                    {leaveDates.map(date => (
                      <span key={date.getTime()} className="bg-white text-orange-700 text-xs px-3 py-1.5 rounded-lg border border-orange-200 flex items-center gap-2 font-bold shadow-sm">
                        {date.toLocaleDateString('th-TH', { day: 'numeric', month: 'short', year: '2-digit' })}
                        <button type="button" onClick={() => setLeaveDates(prev => prev.filter(d => d.toDateString() !== date.toDateString()))} className="text-gray-400 hover:text-red-500 transition-colors">
                          <X size={14} />
                        </button>
                      </span>
                    ))}
              ...
              <div className="bg-orange-50 p-4 rounded-xl border border-orange-100 text-sm text-orange-800">
                <p>วันที่เลือกจะถูกตั้งค่า <strong>ชม. = 0</strong> ในรายการ outside-work-special ของเจ้าหน้าที่คนนี้</p>
              </div>
            </form>

            <div className="bg-gray-50 p-6 border-t border-gray-100 flex gap-4 flex-shrink-0">
              <button type="button" onClick={() => setIsLeaveModalOpen(false)} className="flex-1 py-3 bg-gray-100 text-gray-700 rounded-xl font-bold hover:bg-gray-200 border border-gray-300 transition-all">ยกเลิก</button>
              <button onClick={handleSubmitLeave} type="button" className="flex-[2] py-3 bg-orange-600 text-white rounded-xl font-bold hover:bg-orange-700 transition-all shadow-lg shadow-orange-600/20">บันทึกวันลา</button>
            </div>
          </div>
        </div>
      )}
                        ...
                        <tr key={title.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-center">
                            <input
                              ...
                              onChange={() => setSelectedReportTitleId(title.id)}
              ...
              <button
                onClick={async () => {
                  ...
                  if (approver) {
                    ...
                    try {
                      await axios.post('/dcms/api/admin/report_configuration.php?action=save_approver', {
      ...
      <ConfirmModal
        ...
        size={confirmModal.title === 'ยืนยันการลงเวลาอัตโนมัติ (กรณีพิเศษ)' ? 'xl' : 'sm'}
      />
    </div>
  );
};

export default OutsideWorkSpecialManagement;
